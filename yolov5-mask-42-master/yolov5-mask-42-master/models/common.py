...
class TransformerLayerplus(nn.Module):
    def __init__(self, c, num_heads, drop_prob=0.1):
        super().__init__()
    ...
    def forward(self, x):
        x = self.drop_path(self.ma(self.q(x), self.k(x), self.v(x))[0]) + x
        x = self.drop_path(self.fc2(F.gelu(self.fc1(x)))) + x
        return x
...
class TransformerBlockplus(nn.Module):
    ...
    def __init__(self, c1, c2, num_heads, num_layers):
        ...
        if c1 != c2:
            self.conv = Conv(c1, c2)
        self.linear = nn.Linear(c2, c2)  # learnable position embedding
        self.tr = nn.Sequential(*(TransformerLayerplus(c2, num_heads) for _ in range(num_layers)))
        self.c2 = c2
